...
class MembershipUpdateSpec extends Specification {
    @Autowired
    ...
    void "it should return not authorized when token is not provided"() {
        ...
        when:
        def response = mockMvc.perform(put("/api/memberships/1/")
    ...
    void "it should return not found when membership does not exist"() {
        ...
        when:
        def response = mockMvc.perform(put("/api/memberships/${membershipId}/")
                .contentType(MediaType.APPLICATION_JSON)
                .content(asJsonString(info))
                .header("Authorization", "Bearer ${token.getToken()}"))
                .andReturn().response
    ...
    void "it should return forbidden when auth user is not owner of membership"() {
        ...
        when:
        def response = mockMvc.perform(put("/api/memberships/${membership.id()}/")
                .contentType(MediaType.APPLICATION_JSON)
                .content(asJsonString(info))
                .header("Authorization", "Bearer ${token.getToken()}"))
                .andReturn().response
    ...
    void "it should return accepted when membership is updated successfully"() {
        given:
        def membership = new MembershipBuilder().build()
        officeHolderRepo.store(membership.officeBranch().owner())
        officeBranchRepo.store(membership.officeBranch())
    ...
    void "it should return accepted when membership is updated successfully by a collaborator"() {
        given:
        def membership = new MembershipBuilder().build()
        officeHolderRepo.store(membership.officeBranch().owner())
        officeBranchRepo.store(membership.officeBranch())
