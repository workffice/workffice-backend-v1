...
class NewsSenderSpec extends Specification {
    @Autowired
    MockMvc mockMvc
    @Autowired
    ...
    @Autowired
    news.domain.OfficeBranchRepository officeBranchNewsRepo
    @Autowired
    AuthUtil authUtil
    @Autowired
    ...
    void "it should return not authorized when token is not provided"() {
        when:
        def response = mockMvc.perform(post("/api/news/1/messages/"))
                .andReturn().response
    ...
    void "it should return forbidden when auth user is not owner of news"() {
        ...
        when:
        def response = mockMvc.perform(post("/api/news/${news.id()}/messages/")
                .header("Authorization", "Bearer ${token.getToken()}"))
                .andReturn().response
    ...
    void "it should return forbidden when collaborator has no enough perms to send news"() {
        given:
        def officeBranch = new OfficeBranchBuilder().build()
        officeHolderRepo.store(officeBranch.owner())
    ...
    void "it should return accepted when news is sent successfully"() {
        given:
        def officeBranch = new OfficeBranchBuilder().build()
        officeHolderRepo.store(officeBranch.owner())
        officeBranchRepo.store(officeBranch)
        def officeBranchNews = new OfficeBranch(officeBranch.id().toString())
        officeBranchNewsRepo.store(officeBranchNews)
        def news = new NewsBuilder()
                .withOfficeBranchId(officeBranch.id().toString())
                .build()
        newsRepo.store(news)
    ...
    void "it should return accepted when news is sent successfully by a collaborator with right perms"() {
        given:
        def officeBranch = new OfficeBranchBuilder().build()
        officeHolderRepo.store(officeBranch.owner())
        officeBranchRepo.store(officeBranch)
